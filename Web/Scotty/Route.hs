...
matchAny pattern action = ScottyT $ MS.modify $ \s -> addRoute (route (routesOptions s) (handler s) Nothing pattern action) s
...
addroute method pat action = ScottyT $ MS.modify $ \s -> addRoute (route (routesOptions s) (handler s) (Just method) pat action) s
...
route opts h method pat action app req =
    ...
    in if methodMatches
       then case matchRoute pat req of
            Just captures -> do
                env <- liftIO $ catch (Right <$> mkEnv req captures (maxRequestSize opts)) (\ex -> return . Left $ ex)
...
mkEnv :: forall m. MonadIO m => Request -> [Param] -> Maybe Kilobytes ->m ActionEnv
mkEnv req captures maxSize = do
    ...
    let rbody = getRequestBodyChunk req
        ...
        bs = do
          ...
          case state of
            ...
            BodyUntouched ->
              do chunks <- readRequestBody rbody return maxSize
